...
{
    /*** begin the select ***/
    $dropdown = '<select style="width: 300px; height: 27px;" multiple autofocus name="'.$name.'[]" id="'.$name.'">'."\n";
...
<?php //echo " entx ". print_r($_REQUEST['sel_ent']); ?>
...
<a href="../index.php"><i class="fa fa-home" style="font-size:14pt; margin-left:25px;"></i><span></span></a>
	...
	<div id="titulo_graf"> <?php echo __('Tickets', 'dashboard') ?> </div>
		...
		<tr><td height="17px"></td></tr>			
		<tr>
			<td style="margin-top:2px; width:100px;"><?php echo __('Group'); ?>:  </td>		
			<td style="margin-top:2px;">
			<?php
			...
			while ($row_grp = $DB->fetch_assoc($result_grp))		
				{ 
					$v_row_grp = $row_grp['id'];
					$arr_grp[$v_row_grp] = $row_grp['name'] ;			
				} 
...
if(!isset($_REQUEST["sel_ent"])) { 
	...
	$id_ent = implode(',',$_REQUEST["sel_ent"]); 
	}
...
if(isset($_REQUEST["sel_sta"]) && $_REQUEST["sel_sta"] != '0') { 
	...
	$id_sta = "AND glpi_tickets.status IN (". implode(',',$_REQUEST["sel_sta"]) .")" ;
	}
}
else { $id_sta = ''; }
...
if(isset($_REQUEST["sel_req"]) && $_REQUEST["sel_req"] != '0') { 
	$id_req = "AND glpi_tickets.requesttypes_id IN (" . implode(',',$_REQUEST["sel_req"]) .")"; 
	}
else { $id_req = ''; }

if(isset($_REQUEST["sel_pri"]) && $_REQUEST["sel_pri"] != '0') { 
	$id_pri = "AND glpi_tickets.priority IN (". implode(',',$_REQUEST["sel_pri"]) .")"; 
	}
else { $id_pri = ''; }

if(isset($_REQUEST["sel_cat"]) && $_REQUEST["sel_cat"] != '0') { 
	$id_cat = "AND glpi_tickets.itilcategories_id IN (" . implode(',',$_REQUEST["sel_cat"]) .")"; 
	}
else { $id_cat = ''; }

if(isset($_REQUEST["sel_tip"]) && $_REQUEST["sel_tip"] != '0') { 
	$id_tip = "AND glpi_tickets.type IN (" . implode(',',$_REQUEST["sel_tip"]) .")"; 
	}
else { $id_tip = ''; }
...
//grupos
if(isset($_REQUEST["sel_grp"]) && $_REQUEST["sel_grp"] != '0') { 
	$id_grp = "AND glpi_groups_tickets.groups_id IN (" . implode(',',$_REQUEST["sel_grp"]) .")  AND glpi_groups_tickets.tickets_id = glpi_tickets.id";
	$from = "FROM glpi_tickets, glpi_groups_tickets"; 
	}
else { $id_grp = ''; }
...
// Chamados
$sql_cham = 
"SELECT glpi_tickets.id, entities_id, glpi_tickets.name, glpi_tickets.date, closedate, solvedate, glpi_tickets.status, users_id_recipient, requesttypes_id, 
glpi_tickets.itemtype, priority, itilcategories_id, glpi_tickets.type, due_date 
". $from ."
WHERE glpi_tickets.entities_id IN (". $id_ent .")
...
$consulta1 = 
"SELECT COUNT(glpi_tickets.id) AS total1, glpi_tickets.id AS total
". $from ."
WHERE glpi_tickets.entities_id IN (". $id_ent .")
AND glpi_tickets.is_deleted = 0
". $period ."
...
$conta_cons = $DB->result($result_cons1,0,'total1');
...
//listar chamados
echo "
<div class='well info_box row-fluid col-md-12 report-tic' style='margin-left: -1px; margin-top: -30px;'>
...
while($row = $DB->fetch_assoc($result_cham)){
	...
	//vencimento	
	//check due_date	
	$sql_due = "SELECT glpi_tickets.due_date, glpi_tickets.closedate 
	FROM glpi_tickets
