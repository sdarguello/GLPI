...
if($sel_ent == '' || $sel_ent == -1) {	
	...
	$entidade_sw = "WHERE entities_id IN (".$ent.") ";
	$entidade1 = "";
	...
	$entidade_sw = "WHERE entities_id IN (".$sel_ent.") ";
}
?>
	...
	<tr>	
		<table border=0 width='400px' align='left' >
			<tr style='font-size: 16px; font-weight:bold; vertical-align:middle;'>
				<td> ". __('Within','dashboard')  ." : ". $w ." - (". round(($w*100)/$consulta,0) ." %) </td>
				<td> ". __('Overdue','dashboard') ." : ". $v ." - (". round(($v*100)/$consulta,0) ." %) </td>
			</tr>	
			<tr><td>&nbsp;</td></tr>	
		</table>					
	</tr>
